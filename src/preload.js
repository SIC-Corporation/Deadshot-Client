...
const injectNexa = () => {
    // Only inject if body exists and we haven't already injected
    if (!document.body || document.getElementById('nexa-overlay-container')) return;

    console.log("NexaFlow: Safe Injection Active.");

    const style = document.createElement('style');
    style.textContent = `
        /* KILL ADS AND CLEAN UI */
        #ad-container, .ad-unit, .home-right, .home-left, #ui-pause, .ad-v { 
    ...
    container.innerHTML = `
        ...
        <div id="nexa-esc-menu" class="nexa-menu-card">
            <h2 style="margin: 0; color: #ff4757; letter-spacing: 2px;">PAUSED</h2>
            <p style="font-size: 11px; opacity: 0.6; margin-bottom: 20px;">SIC CORP // SYSTEM OVERRIDE</p>
            ...
            <button class="nexa-btn" style="background: #111 !important;" onclick="ipcRenderer.send('app-quit-action')">EXIT CLIENT</button>
        </div>
    `;
...
// Check every 100ms if the page is ready for injection
